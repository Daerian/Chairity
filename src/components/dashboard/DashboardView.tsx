...
import { Plus, LogOut, Calendar, ChevronRight, Trash2 } from 'lucide-react'
...
export default function DashboardView({ user, events: initialEvents }: Props) {
        ...
        {events.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {events.map((event) => (
              <div
                key={event.id}
                className="group relative bg-white rounded-2xl border border-event-border shadow-card hover:shadow-card-hover transition-all overflow-hidden"
              >
                {/* Colored accent strip */}
                <div className="h-1.5 bg-gradient-to-r from-gold-400 to-gold-600" />
                <div className="p-5">
                  <h3 className="font-display text-lg font-semibold text-gray-800 truncate pr-6">
                    {event.name}
                  </h3>
                  {event.event_date && (
