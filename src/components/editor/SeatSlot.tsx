...
export default function SeatSlot({ tableId, seatNumber, assignedGuest, guestMap, assignmentBySeat, onUnassign }: Props) {
  ...
  const draggableId = assignedGuest ? `seated-${assignedGuest.id}` : `empty-seat-${tableId}-${seatNumber}`
  const { attributes, listeners, setNodeRef: setDragRef, transform, isDragging } = useDraggable({
    id: draggableId,
    data: assignedGuest
      ? { type: 'seated', guestId: assignedGuest.id, tableId, seatNumber }
      : { type: 'empty' },
  ...
  // Unassigned guests available for dropdown — all guests not currently seated
  // We derive this from guestMap minus those in assignmentBySeat
  const seatedGuestIds = new Set(assignmentBySeat.values())
  const availableGuests = Array.from(guestMap.values()).filter(
    (g) => !seatedGuestIds.has(g.id) || g.id === assignedGuest?.id
  )
  ...
  return (
    <div className="relative">
      <div
        ...
        className={`
          flex items-center gap-1 px-2 py-1.5 rounded-md border text-xs min-h-[30px] transition-all select-none
        ...
        {assignedGuest ? (
          <>
            ...
            <button
              ...
              className="ml-0.5 shrink-0 text-gray-300 hover:text-red-400 transition-colors"
            >
              <X size={10} />
            </button>
          </>
      ...
      {/* Dropdown for selecting a guest */}
              ...
              availableGuests.map((g) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    // Emit a custom event that EditorLayout can pick up, or we can use a callback
                    document.dispatchEvent(
                      new CustomEvent('chairity:assign', {
                        detail: { guestId: g.id, tableId, seatNumber },
                      })
                    )
                  }}
                >
