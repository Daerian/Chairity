...
export default function TableConfigModal({ eventId, tables, onClose, onSave }: Props) {
  ...
  function addBulk() {
    ...
    const newRows: TableRow[] = Array.from({ length: bulkCount }, (_, i) => ({
      id: null, name: `${bulkPrefix} ${startIndex + i}`, capacity: bulkCapacity, sort_order: startIndex + i - 1,
    }))
  ...
  function markDelete(idx: number) {
    const row = rows[idx]
    if (row.id === null) setRows((prev) => prev.filter((_, i) => i !== idx))
    else updateRow(idx, { toDelete: !row.toDelete })
  }

  async function handleSave() {
    const activeRows = rows.filter((r) => !r.toDelete)
    if (activeRows.some((r) => !r.name.trim())) { setError('All tables must have a name.'); return }
    setError('')
    setSaving(true)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="bg-gold-50 rounded-xl p-4 border border-gold-200">
            ...
            <div className="grid grid-cols-3 gap-3">
              <label className="space-y-1">
                ...
                <input type="number" min={1} max={200} value={bulkCount} onChange={(e) => setBulkCount(Math.max(1, parseInt(e.target.value) || 1))} className="w-full px-2 py-1.5 text-sm border border-gray-200 rounded-lg outline-none focus:border-gold-400" />
              </label>
              <label className="space-y-1">
                <span className="text-xs text-event-muted">Seats each</span>
                <input type="number" min={1} max={50} value={bulkCapacity} onChange={(e) => setBulkCapacity(Math.max(1, parseInt(e.target.value) || 1))} className="w-full px-2 py-1.5 text-sm border border-gray-200 rounded-lg outline-none focus:border-gold-400" />
              </label>
              <label className="space-y-1">
                <span className="text-xs text-event-muted">Name prefix</span>
                <input type="text" value={bulkPrefix} onChange={(e) => setBulkPrefix(e.target.value)} className="w-full px-2 py-1.5 text-sm border border-gray-200 rounded-lg outline-none focus:border-gold-400" />
              </label>
            </div>
            <button onClick={addBulk} className="mt-3 flex items-center gap-1.5 px-3 py-1.5 bg-gold-500 text-white text-sm rounded-lg hover:bg-gold-600 transition-colors">
              <Plus size={14} />Add {bulkCount} table{bulkCount !== 1 ? 's' : ''} of {bulkCapacity}
            </button>
          </div>
          ...
          {rows.length > 0 && (
            <div>
              ...
              <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                {rows.map((row, idx) => (
                  <div key={row.id ?? `new-${idx}`} className={`flex items-center gap-2 p-2 rounded-lg border transition-all ${row.toDelete ? 'opacity-40 border-red-200 bg-red-50' : 'border-gray-200 bg-white'}`}>
                    <input value={row.name} onChange={(e) => updateRow(idx, { name: e.target.value })} disabled={row.toDelete} className="flex-1 text-sm px-2 py-1 border border-transparent rounded focus:border-gold-300 outline-none bg-transparent" />
                    <div className="flex items-center gap-1 shrink-0">
                      <span className="text-xs text-event-muted">seats:</span>
                      <input type="number" min={1} max={50} value={row.capacity} onChange={(e) => updateRow(idx, { capacity: Math.max(1, parseInt(e.target.value) || 1) })} disabled={row.toDelete} className="w-12 text-sm text-center px-1 py-1 border border-gray-200 rounded focus:border-gold-300 outline-none" />
                    </div>
          ...
          {error && <div className="flex items-center gap-2 text-sm text-red-600 bg-red-50 px-3 py-2 rounded-lg"><AlertTriangle size={14} />{error}</div>}
        </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-event-border">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleSave} disabled={saving} className="px-4 py-2 text-sm bg-gold-500 text-white rounded-lg hover:bg-gold-600 disabled:opacity-60 transition-colors">
