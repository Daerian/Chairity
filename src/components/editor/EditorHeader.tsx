...
import { Settings2, Upload, ChevronLeft, Check, Loader2 } from 'lucide-react'
...
export default function EditorHeader({
  eventName, saving, tables, guests, assignments,
  onRename, onOpenTableConfig, onOpenCSVImport,
}: Props) {
    ...
    <header className="flex items-center gap-3 px-4 py-3 bg-white border-b border-event-border shadow-sm z-10 shrink-0">
      ...
      <div className="flex-1 flex items-center gap-2 min-w-0">
        {editing ? (
          ...
          <button
            ...
            className="font-display text-xl font-semibold truncate hover:text-gold-700 transition-colors text-left"
      ...
      <div className="flex items-center gap-2 shrink-0">
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-lg border border-event-border hover:border-gold-400 hover:bg-gold-50 transition-all"
        >
          <Upload size={14} />
          Import CSV
        </button>
