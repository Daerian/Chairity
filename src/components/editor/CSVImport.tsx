...
export default function CSVImport({ eventId, existingNames, onClose, onImport }: Props) {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (parsed.headers.length === 0) { setError('No columns found in CSV.'); return }
      setHeaders(parsed.headers)
      setRows(parsed.rows)
      setFileName(file.name)
      setSelectedColumn(parsed.headers.find((h) => /name/i.test(h)) ?? parsed.headers[0])
    } catch {
  ...
  async function handleImport() {
    ...
    setImporting(true)
    const { data, error: dbError } = await supabase.from('guests').insert(newNames.map((name) => ({ event_id: eventId, name }))).select()
    setImporting(false)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col">
        ...
        <div className="p-6 space-y-5">
          <div onClick={() => inputRef.current?.click()} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) handleFile(f) }} className="border-2 border-dashed border-gold-300 rounded-xl p-8 text-center cursor-pointer hover:bg-gold-50 hover:border-gold-400 transition-all">
          ...
          {headers.length > 0 && (
            <div className="space-y-1.5">
              ...
              <select value={selectedColumn} onChange={(e) => setSelectedColumn(e.target.value)} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg outline-none focus:border-gold-400 bg-white">
          ...
          {names.length > 0 && (
            <div className="bg-gray-50 rounded-xl p-4 space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                {duplicateCount > 0 && <span className="text-xs text-amber-600 flex items-center gap-1"><AlertTriangle size={12} />{duplicateCount} already exist (skipped)</span>}
              </div>
              <div className="max-h-36 overflow-y-auto space-y-0.5">
                {newNames.slice(0, 50).map((name) => (
                  <div key={name} className="flex items-center gap-2 text-xs text-gray-600 py-0.5">
                    <Check size={10} className="text-green-400 shrink-0" />{name}
                  </div>
                ))}
                {newNames.length > 50 && <p className="text-xs text-event-muted pt-1">…and {newNames.length - 50} more</p>}
              </div>
            </div>
          )}
        ...
        <div className="flex justify-end gap-2 px-6 py-4 border-t border-event-border">
          ...
          <button onClick={handleImport} disabled={newNames.length === 0 || importing} className="px-4 py-2 text-sm bg-gold-500 text-white rounded-lg hover:bg-gold-600 disabled:opacity-50 transition-colors">
