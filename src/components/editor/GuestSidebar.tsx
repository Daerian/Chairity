    ...
    <aside className="w-64 shrink-0 flex flex-col bg-white border-r border-event-border overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-event-border bg-gold-50">
        <div className="flex items-center gap-2 text-xs text-event-muted">
          <Users size={13} />
          <span>
            <strong className="text-gray-700">{assignedCount}</strong> / {totalGuests} seated
          </span>
        ...
        <div className="mt-2 h-1.5 bg-gold-100 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: totalGuests ? `${(assignedCount / totalGuests) * 100}%` : '0%' }}
          />
        </div>
      </div>
      ...
      {/* Guest list — also a drop zone to unassign */}
      <div
        ref={setNodeRef}
            ...
            {guests.length === 0 ? (
              <>
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 text-sm bg-gold-500 text-white rounded-lg hover:bg-gold-600 transition-colors"
                >
                  <Upload size={13} />
                  Import CSV
                </button>
      ...
      {guests.length > 0 && (
        <div className="p-3 border-t border-event-border">
          <button
            onClick={onOpenCSVImport}
            className="w-full flex items-center justify-center gap-1.5 px-3 py-2 text-sm border border-dashed border-gold-300 text-gold-600 rounded-lg hover:bg-gold-50 transition-colors"
          >
            <Upload size={13} />
            Import more guests
          </button>
