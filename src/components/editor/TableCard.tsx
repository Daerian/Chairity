    ...
    <div className="bg-white rounded-xl border border-event-border shadow-card hover:shadow-card-hover transition-shadow flex flex-col">
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-event-border bg-gold-50 rounded-t-xl">
        {editingName ? (
          <div className="flex items-center gap-1 flex-1 min-w-0">
            <input autoFocus value={nameDraft} onChange={(e) => setNameDraft(e.target.value)} onBlur={commitName} onKeyDown={(e) => { if (e.key === 'Enter') commitName(); if (e.key === 'Escape') { setNameDraft(displayName); setEditingName(false) } }} className="flex-1 min-w-0 text-sm font-semibold bg-white border border-gold-300 rounded px-1.5 py-0.5 outline-none" />
          ...
          <>
            ...
            <button onClick={() => setEditingName(true)} className="text-gray-400 hover:text-gold-600 transition-colors shrink-0"><Pencil size={12} /></button>
          </>
        )}
      ...
      <div className="p-3 grid gap-1.5" style={{ gridTemplateColumns: 'repeat(2, 1fr)' }}>
        {Array.from({ length: table.capacity }, (_, i) => i + 1).map((seatNum) => {
          ...
          return (
            <SeatSlot key={seatNum} tableId={table.id} seatNumber={seatNum} assignedGuest={guest} guestMap={guestMap} assignmentBySeat={assignmentBySeat} onUnassign={() => guestId && onUnassign(guestId)} />
          )
        })}
      </div>
    </div>
